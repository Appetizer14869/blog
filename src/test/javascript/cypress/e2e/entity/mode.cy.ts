...
describe('Mode e2e test', () => {
  ...
  const modeSample = { name: 'blah', handle: 'welcome' };
  ...
  describe('new Mode page', () => {
    ...
    it('should create an instance of Mode', () => {
      cy.get(`[data-cy="name"]`).type('worth circle aha');
      cy.get(`[data-cy="name"]`).should('have.value', 'worth circle aha');

      cy.get(`[data-cy="handle"]`).type('unless coaxingly candid');
      cy.get(`[data-cy="handle"]`).should('have.value', 'unless coaxingly candid');
